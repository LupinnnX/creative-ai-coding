...
export async function createCodebase(data: {
  ...
  const assistantType = data.ai_assistant_type || 'droid';
...
export async function findCodebaseByName(name: string): Promise<Codebase | null> {
  ...
  const result = await pool.query<Codebase>(
    'SELECT * FROM remote_agent_codebases WHERE LOWER(name) = LOWER($1) ORDER BY updated_at DESC LIMIT 1',
    [trimmed]
  );
  return result.rows[0] || null;
}

export async function listCodebases(): Promise<Codebase[]> {
  const result = await pool.query<Codebase>(
    'SELECT * FROM remote_agent_codebases ORDER BY updated_at DESC, created_at DESC'
  );
  return result.rows;
}
